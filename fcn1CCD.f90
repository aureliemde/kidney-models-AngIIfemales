...
!	 in P, A, B, and E	   
!    It is used to compute values at the entry of the CCD.
!---------------------------------------------------------------------72
!---------------------------------------------------------------------72
	   
subroutine fcn1CCD(n,x,fvec,iflag,numpar,pars,tube,id)
...
!	 Assign concentrations, volumes, and potentials in lumen and blood
  Do J = 1, NS
     C(J,1) = tube%conc(J,1) !C0ccd(J,1)
     C(J,6) = tube%conc(J,6) !C0ccd(J,6)
  End Do
  EP(6) = tube%ep(6)  !EP0ccd(6)
...
!---------------------------------------------------------------------72
  ...
  fvec(47) = S(47)+S(43)-S(15)-S(27)	 !pH in B
...
!---------------------------------------------------------------------72
  ...
  CimpP=CPimprefCCD*volPrat
  CimpA=CAimprefCCD*VolArat
  CimpB=CBimprefCCD*VolBrat
  
  facP=dexp(dlog(10.0d0)*(ph(2)-pKbuf))
  facA=dexp(dlog(10.0d0)*(ph(3)-pKbuf))
  facB=dexp(dlog(10.0d0)*(ph(4)-pKbuf))
  CbufP=CPbuftotCCD*volPrat*facP/(facP+1)
  CbufA=CAbuftotCCD*volArat*facA/(facA+1)
  CbufB=CBbuftotCCD*volBrat*facB/(facB+1)
  
  elecP=zPimpCCD*CimpP-CbufP
  elecA=zAimpCCD*CimpA-CbufA
  elecB=zBimpCCD*CimpB-CbufB
  elecE=0.0d0
  do I=1,NS
